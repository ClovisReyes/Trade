--[[
    ====================================================================================
    🐟 FISH IT - COMPLETE DIRECT ACTION & REMOTE PACKET SPY 🕵️‍♂️ (UNDETECTED)
    ====================================================================================
    Menangkap SEMUA Aksi Asli (Outgoing & Incoming):
    1. 📤 OUTGOING RemoteFunction:
       - SendTradeOffer (Bagaimana dia kirim offer)
       - AcceptTradeOffer (Bagaimana dia auto-accept / klik yes)
       - AddItem (Bagaimana dia memasukkan ikan dan UUID-nya)
       - SetReady (Bagaimana dia pencet Ready)
       - ConfirmTrade (Bagaimana dia pencet Confirm)
       - CancelTrade / Decline (Bagaimana dia cancel)
    2. 📥 INCOMING RemoteEvent (Respon dari server)
    3. 🎒 INVENTORY Replion (Perubahan isi tas)
    4. 🔒 100% Anti-Cheat Safe (Tanpa hook VIM / PlayerGui).
    ====================================================================================
--]]
...
----------------------------------------------------------------------
-- 1. SAFE OUTGOING REMOTE SPY (Menangkap SendOffer, Ready, Confirm, AddItem, Accept)
----------------------------------------------------------------------
local function setup_outgoing_remote_spy()
    ...
    old_namecall = hookmetamethod(game, "__namecall", newcclosure(function(self, ...)
        ...
        if is_capturing and self and typeof(self) == "Instance" then
            ...
            local is_trade = string.find(sname, "Trading", 1, true) or string.find(sname, "Trade", 1, true) or (self.Parent and string.find(self.Parent.Name, "Trading", 1, true))
            ...
            if is_trade then
                ...
                if method == "InvokeServer" and self:IsA("RemoteFunction") then
                    task.spawn(function()
                        ...
                        local detail = string.format("Aksi Script: Memanggil RemoteFunction\nPath: %s\nParameter yang dikirim:\n%s", self:GetFullName(), arg_str)
                        record_log("OUTGOING RF", sname .. ":InvokeServer()", detail, Color3.fromRGB(56, 189, 248))
                    end)
                elseif method == "FireServer" and self:IsA("RemoteEvent") then
                    task.spawn(function()
                        ...
                        local arg_str = #arg_lines > 0 and table.concat(arg_lines, "\n") or "  (Tanpa Parameter)"
                        local detail = string.format("Aksi Script: Menembak RemoteEvent\nPath: %s\nParameter:\n%s", self:GetFullName(), arg_str)
                        record_log("OUTGOING RE", sname .. ":FireServer()", detail, Color3.fromRGB(168, 85, 247))
                    end)
                end
            end
        end
...
----------------------------------------------------------------------
-- 2. INCOMING SERVER EVENT SNIFFER
----------------------------------------------------------------------
            ...
            if child:IsA("RemoteEvent") and (string.find(child.Name, "Trading", 1, true) or string.find(child.Name, "Trade", 1, true)) then
                child.OnClientEvent:Connect(function(...)
                    ...
                    task.spawn(function()
                        ...
                        local detail = string.format("Event dari Server: %s\nData:\n%s", child.Name, table.concat(arg_lines, "\n"))
...
local function build_spy_gui()
    ...
    pcall(function()
        for _, c in ipairs(parent_gui:GetChildren()) do
            if c.Name == "FishIt_DirectSpy" then c:Destroy() end
        end
    end)

    local gui = Instance.new("ScreenGui")
    gui.Name = "FishIt_DirectSpy"
    ...
    title.Text = "🕵️‍♂️ Fish It - Action & Remote Invocation Spy"
    ...
    copy_btn.MouseButton1Click:Connect(function()
        local all_lines = {
            "==================================================",
            "       FISH IT DIRECT ACTION & PACKET LOG",
        ...
        pcall(function()
            if writefile then writefile("fishit_direct_action_log.txt", full_text) end
        end)
    ...
    record_log("SYSTEM", "Spy Ready", "Action Spy aktif.\nMenangkap pemanggilan SendOffer, Accept, AddItem, SetReady, dan ConfirmTrade saat script Luraph jalan...", Color3.fromRGB(240, 240, 240))
end

-- Initialize safely
pcall(setup_outgoing_remote_spy)
