--[[
    ====================================================================================
    🐟 FISH IT - ULTIMATE BULLETPROOF TRADE PACKET & INVOCATION SPY 🕵️‍♂️
    ====================================================================================
    Perbaikan Kunci:
    1. Case-Insensitive Namecall (`invokeserver` / `InvokeServer`) agar terbaca di semua executor mobile.
    2. Dynamic Remote Attachment via `DescendantAdded` di ReplicatedStorage.
    3. 100% Anti-Cheat Safe (Zero BAC Kick: Tanpa hook VIM / PlayerGui).
    4. Menangkap: SendTradeOffer, AcceptTradeOffer, AddItem, SetReady, ConfirmTrade, Cancel.
    ====================================================================================
--]]
...
----------------------------------------------------------------------
-- 1. UNIVERSAL CASE-INSENSITIVE REMOTE INTERCEPTOR
----------------------------------------------------------------------
local function log_remote_call(inst, call_type, args)
    if not is_capturing or not inst then return end
    local sname = tostring(inst.Name)
    local slname = string.lower(sname)
    local parent_name = inst.Parent and tostring(inst.Parent.Name) or ""
    ...
    if string.find(slname, "trade", 1, true) or 
       ...
       string.find(parent_name, "Trading", 1, true) then
        ...
        local detail = string.format("Path: %s\nParameter Terkirim:\n%s", inst:GetFullName(), arg_str)
        
        record_log("OUTGOING " .. call_type, sname .. ":" .. call_type .. "()", detail, Color3.fromRGB(56, 189, 248))
    end
end

local function setup_all_remote_interceptors()
    local newcclosure = newcclosure or function(f) return f end
    local getnamecallmethod = getnamecallmethod or get_namecall_method

    -- Hook Layer 1: __namecall (Case-Insensitive for Mobile Executors)
    if hookmetamethod then
        local old_namecall
        old_namecall = hookmetamethod(game, "__namecall", newcclosure(function(self, ...)
            local raw_method = getnamecallmethod() or ""
            local method = string.lower(tostring(raw_method))

            if self and typeof(self) == "Instance" then
                if method == "invokeserver" and self:IsA("RemoteFunction") then
                    local args = {...}
                    task.spawn(log_remote_call, self, "InvokeServer", args)
                elseif method == "fireserver" and self:IsA("RemoteEvent") then
                    local args = {...}
                    task.spawn(log_remote_call, self, "FireServer", args)
                end
            end
...
----------------------------------------------------------------------
-- 2. DYNAMIC INCOMING SERVER EVENT SNIFFER
----------------------------------------------------------------------
...
local function attach_listener(rem)
    ...
    local slname = string.lower(rem.Name)
    if not (string.find(slname, "trade", 1, true) or string.find(slname, "item", 1, true)) then return end
    ...
    pcall(function()
        rem.OnClientEvent:Connect(function(...)
            ...
            task.spawn(function()
                ...
                local detail = string.format("RemoteEvent: %s\nData:\n%s", rem.Name, table.concat(arg_dump, "\n"))
                record_log("SERVER EVENT", rem.Name, detail, Color3.fromRGB(34, 197, 94))
            end)
        end)
    end)
end
...
local function setup_trade_event_listeners()
    for _, desc in ipairs(replicated_storage:GetDescendants()) do
        if desc:IsA("RemoteEvent") then attach_listener(desc) end
    end
...
----------------------------------------------------------------------
-- 3. ATTRIBUTE & STATE SNIFFER (IsTrading)
----------------------------------------------------------------------
...
local function build_spy_gui()
    ...
    pcall(function()
        for _, c in ipairs(parent_gui:GetChildren()) do
            if c.Name == "FishIt_CompleteSpy" then c:Destroy() end
        end
    end)

    local gui = Instance.new("ScreenGui")
    gui.Name = "FishIt_CompleteSpy"
    ...
    title.Text = "🕵️‍♂️ Fish It - Outgoing Remote & Packet Spy"
    ...
    copy_btn.MouseButton1Click:Connect(function()
        local all_lines = {
            "==================================================",
            "       FISH IT COMPLETE PACKET & ACTION LOG",
        ...
        pcall(function()
            if writefile then writefile("fishit_complete_packet_log.txt", full_text) end
        end)
    ...
    record_log("SYSTEM", "Spy Active", "Penyadap Outgoing Remote & Server Event Aktif (Case-Insensitive Mode).", Color3.fromRGB(240, 240, 240))
end

-- Initialize safely
pcall(setup_all_remote_interceptors)
